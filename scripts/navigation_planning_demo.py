...
# Specify grid-frame coordinates for the rooms' center (for rotation)
GRID_T_ROOMS_X_M = GRID_WIDTH_M / 2
GRID_T_ROOMS_Y_M = GRID_HEIGHT_M / 2
...
def create_wall_kinematic_state(
    ...
    :param grid_t_local: Transform from the grid frame to local/room coordinates
    ...
    local_t_wall = Pose2D(x=center_x, y=center_y, yaw_rad=0.0)
    grid_t_wall = grid_t_local @ local_t_wall

    return ObjectKinematicState(name, grid_t_wall.to_3d(), wall_collision_model)


def create_door_kinematic_state(grid_t_local: Pose2D) -> ObjectKinematicState:
    """Create an ObjectKinematicState for a door that blocks the doorway.

    :param grid_t_local: Grid-to-local frame transform
    ...
    local_t_door = Pose2D(x=DIVIDING_WALL_X_M, y=0.0, yaw_rad=0.0)
    grid_t_door = grid_t_local @ local_t_door

    return ObjectKinematicState(
        name="door",
        pose=grid_t_door.to_3d(),
        collision_model=door_collision_model,
    )


def create_room_environment(rotation_rad: float, grid_frame: str = "grid") -> RoomsEnvironment:
    ...
    :param grid_frame: Name of the reference frame of the occupancy grid
    ...
    grid_t_local = Pose2D(
        x=GRID_T_ROOMS_X_M,
        y=GRID_T_ROOMS_Y_M,
        yaw_rad=rotation_rad,
        ref_frame=grid_frame,
    )
    ...
    walls.append(
        create_wall_kinematic_state(
            name="wall_bottom",
            center_x=0.0,
            center_y=-half_height_m - WALL_THICKNESS_M / 2,
            length_x=ROOM_WIDTH_M,
            length_y=WALL_THICKNESS_M,
            grid_t_local=grid_t_local,
        ),
    )
    ...
    walls.append(
        create_wall_kinematic_state(
            name="wall_left",
            center_x=-half_width_m - WALL_THICKNESS_M / 2,
            center_y=0.0,
    ...
    walls.append(
        create_wall_kinematic_state(
            ...
            center_y=-half_height_m + divider_height_m / 2,
            length_x=WALL_THICKNESS_M,
    ...
    door = create_door_kinematic_state(grid_t_local)

    return RoomsEnvironment(walls=walls, door=door, grid_t_local=grid_t_local)
...
def stamp_walls_into_occupancy(
...
def main() -> None:
    ...
    console.print(
        f"Robot footprint: {ROBOT_MIN_X_M}m to {ROBOT_MAX_X_M}m (x), +/-{ROBOT_HALF_WIDTH_M}m (y)",
    )
    ...
    while True:
        ...
        console.print(
            f"[bold]Step 1:[/bold] Creating room environment with rotation {np.degrees(rotation_rad):.1f} deg...",
        )
        ...
        console.print(
            "[bold]Step 3:[/bold] Sampling start pose (left room) and goal pose (right room)...",
        )
        ...
        console.print(
            f"  Start: ({start_pose.x:.2f}, {start_pose.y:.2f}, {np.degrees(start_pose.yaw_rad):.1f} deg)",
        )
        console.print(
            f"  Goal:  ({goal_pose.x:.2f}, {goal_pose.y:.2f}, {np.degrees(goal_pose.yaw_rad):.1f} deg)",
        )
        console.print()
        ...
        console.print(
            f"  Door pose: ({door_state.pose.position.x:.2f}, {door_state.pose.position.y:.2f}, {door_state.pose.position.z:.2f})",
        )
        console.print(
            f"  Door collision model: Box({DOOR_THICKNESS_M}m x {DOOR_WIDTH_M}m x {DOOR_HEIGHT_M}m)",
        )
        console.print()
        ...
        if path_blocked is None:
            ...
            console.print(
                f"  [yellow]UNEXPECTED:[/yellow] Path found with {len(path_blocked)} waypoints",
            )

        # Step 8: Visualize blocked state
        console.print()
        console.print(
            "[bold]Step 8:[/bold] Visualizing blocked state (press any key to continue)...",
        )
        ...
        console.print()
        console.print(
            "[bold]Step 9:[/bold] Removing door from occupancy grid using rasterizer (door OPEN)...",
        )
        ...
        if path_unblocked is not None:
            console.print(
                f"  [green]VERIFIED:[/green] Path found again with {len(path_unblocked)} waypoints",
            )
        else:
        ...
        console.print()
        console.print(
            "[bold]Step 10:[/bold] Visualizing restored state (press any key to continue)...",
        )
